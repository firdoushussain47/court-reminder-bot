"""
============================================================
  COURT HEARING REMINDER — Twilio WhatsApp + Google Sheets
  Runs automatically via GitHub Actions (no laptop needed!)
============================================================
"""
...
def send_whatsapp_message(twilio_client, phone, name, case, next_date_str, row_number):
    ...
    try:
        ...
        log(f"  Row {row_number}: ✅ Message sent! SID: {message.sid}")
        return True
    except Exception as e:
        error_str = str(e)
        if "21608" in error_str:
            log(f"  Row {row_number}: ❌ {phone} has not joined the Twilio sandbox yet.")
...
def main():
    ...
    for idx, record in enumerate(all_records):
        ...
        log("  ⚠️  Hearing is TOMORROW — sending reminder...")
